...
const mainNavItems: NavItem[] = [
  ...
  { key: 'agent', label: '智能体编排', icon: <Bot size={18} />, page: 'agent' },
      ...
      <div className={`flex items-center gap-2.5 h-12 border-b border-gray-100 dark:border-gray-800 flex-shrink-0 ${collapsed ? 'justify-center px-2' : 'px-4'}`}>
        ...
        {!collapsed && (
          <div className="leading-tight">
            <div className="text-sm font-bold text-gray-900 dark:text-gray-100">智识引擎</div>
            <div className="text-[10px] text-gray-400 dark:text-gray-500">企业知识库平台</div>
          </div>
        )}
      </div>
        ...
        {!collapsed && (
          <>
            <button
              ...
              className="w-full flex items-center gap-3 py-2.5 text-sm transition-colors px-4 text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800 hover:text-gray-900 dark:hover:text-gray-100"
            >
              <span className="flex-shrink-0 text-indigo-500"><Layers size={18} /></span>
              <span className="flex-1 text-left">增强索引</span>
              <ChevronDown size={13} className={`transition-transform text-gray-400 ${showHubs ? 'rotate-180' : ''}`} />
            </button>
            {showHubs && hubItems.map(hub => (
              <button
                key={hub.key}
                onClick={() => onNavigate(hub.page)}
                className={`w-full flex items-center gap-2.5 pl-9 pr-4 py-2 text-sm transition-colors ${
                  hub.page === currentPage
                    ? 'bg-blue-50 dark:bg-blue-950/50 text-blue-700 dark:text-blue-400 border-r-2 border-blue-600 font-medium'
          ...
          {(() => {
            ...
            return (
              <div>
                ...
                {!collapsed && expandedSys && item.children && (
