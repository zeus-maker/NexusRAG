...
> **更新日期**：2026-06-06 · **工程路径**：`rag3-bolt-v1.5/` · **明细文档**：[`前端原型实现进度.md`](./前端原型实现进度.md)

可运行 React 原型（mock 数据）已覆盖以下 Epic 主要界面，供产品走查与开发对齐：

| Epic / 能力域 | 已落地（🟢） | 部分（🟡） | 未开始（⚪） |
|--------------|-------------|-----------|------------|
| 知识库管理 | 列表、创建向导、回收站、配置五 Tab、子导航 | 文档/分块/索引、Hub 壳层 | 导出、文档版本、权限页 |
| 智能对话 | 三栏、设置、Trace、答案对比、纠错 | 查询增强面板 | 高级语法 §4.6 |
| 搜索 / Agent | 搜索四 Tab+设置；Agent 画布+Dataflow | — | React Flow 真实编辑 |
| 评测运营 | 仪表盘、任务、A/B、数据集、满意度 | 成本、回放 | — |
| 工作台 | RAG3 Widget、排行榜、快捷入口 | — | — |
| 系统管理 | — | 用户/角色/监控等基础页 | Prompt/灰度/备份/融合配置 |

**已验证 User Story（原型交互）**：US-1.8 回收站、US-1.9 排行榜、US-2.12 答案对比、US-4.9 满意度、US-4.10 评测数据集。

**变更追溯**：`docs/devlog/2026-06-06-rag3-frontend.md` §1–§9。
