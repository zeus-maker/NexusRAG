...
|----------|----------------|-----------------|-----------|--------|---------|
| E1       | 知识库管理     | 知识域1：文档解析与知识入库 | 16        | P0     | 含   |
...
> **2026-06 设计 enrich**：对齐企业知识运行时（Knowledge Runtime）实践——入库流水线可观测、Chunk 级治理、新鲜度分级、检索层 ACL 保真、五维健康度归因（参考 Atlan/Glean/RAGFlow 行业方案，见 `前端界面实现方案.md` §3.12–§3.15）。
...
#### US-1.13：文档新鲜度分级与陈旧治理

**作为** 平台运营者（李婷），
**我希望能** 为文档分配新鲜度等级并自动检测超期未更新内容，
**以便** 降低「高置信度但已过时」的检索风险，满足合规与运维可观测要求。

**验收标准**：
- [ ] 入库时自动打标新鲜度等级：`realtime`（定价/政策，≤1h）、`daily`、`weekly`、`monthly`；支持按文档类型规则批量配置，管理员可手改单文档
- [ ] 每文档元数据包含：`source_modified_at`、`last_verified_at`、`freshness_tier`、`certification_status`（draft/certified/expired）
- [ ] 陈旧检测：`embed_at - source_modified_at > tier_threshold` 时标黄；超 2× 阈值标红并可配置「检索降权」或「拒检索」
- [ ] 知识库概览展示治理摘要：陈旧文档数、待认证数、解析待复核数；点击跳转对应队列
- [ ] 查询侧（对话/搜索）对陈旧命中可选展示「可能过时」提示（与 Tier 策略联动）
- [ ] 监控指标 `stale_retrieval_rate` 可在系统监控/评测中心查看；超阈值告警
- [ ] 源文档删除或外部同步删除后，30 分钟内清除向量/全文/图谱/Wiki/PageIndex 全部衍生数据（被遗忘权）

#### US-1.14：分块质量运营

**作为** 知识管理员（王芳），
**我希望能** 对分块进行合并/拆分/排除并查看重叠区与质量信号，
**以便** 提升检索命中率、减少「语义相关但答非所问」的误召回。

**验收标准**：
- [ ] 分块卡片支持：合并相邻 Chunk、按标题/句子拆分过大 Chunk、标记「不参与检索」
- [ ] 展开 Chunk 时高亮与相邻 Chunk 的重叠 Token 区（overlap 10–20% 可配置）
- [ ] 每张 Chunk 展示质量信号：策略类型、topical purity 分（0–1）、是否跨章节、是否含表格/代码
- [ ] Chunk 必需元数据：`heading_path`、`chunk_index`、`last_updated`、`owner_id`、检索用 `acl_roles`
- [ ] 分块策略热切换后支持「仅新文档 / 全量重分块」；保留人工干预版本号
- [ ] 人工干预后触发增量重嵌（仅变更 Chunk）；操作写入审计日志
- [ ] 从 Chunk 卡片可「一键带入检索测试」；检索测试结果支持人工标注 ✅相关 / ❌误召回及原因标签
- [ ] 标注结果可沉淀为评测样本（对接 US-4.10）

#### US-1.15：知识库健康度与失败归因

**作为** 平台运营者（李婷），
**我希望能** 在知识库概览与索引状态页看到五维健康分及失败归因，
**以便** 快速派单处理解析/分块/索引/ACL 问题，并与 Hub、监控页形成闭环。

**验收标准**：
- [ ] 知识库级健康分 0–100：向量、全文、PageIndex、图谱、Wiki 五维加权；&lt;70 标黄、&lt;50 告警并推送系统监控
- [ ] 索引状态页五维卡片数据与 PageIndex/Wiki/GraphRAG Hub、系统监控 RAG3 索引 Tab **同源**
- [ ] 失败文档列表按阶段归因：解析 / 分块 / 向量 / PageIndex / 图谱 / Wiki / ACL 投影；展示失败原因与耗时
- [ ] 点击失败项深链：解析失败→§3.4 预览；分块→§3.5；PageIndex→Hub 建树队列；图谱→Hub 建索引队列；ACL→§3.10
- [ ] 概览页展示「进行中流水线」：上传/解析/分块/索引各阶段文档数与 ETA（与文档列表上传队列一致）
- [ ] 支持按健康分、失败数、陈旧数排序知识库列表

#### US-1.16：外部数据源 ACL 保真

**作为** 安全管理员，
**我希望能** 外部数据源同步时保留源系统 ACL 并记录版本，
**以便** Chunk 级检索权限与源系统一致，满足审计与合规要求。

**验收标准**：
- [ ] 数据源配置支持 `sync_mode`：`batch`（全量 crawl + ACL 投影）/ `live`（查询时 OAuth 拉源，源系统鉴权）
- [ ] 批量模式：同步前可配置 `field_exclusions`；入库 Chunk 附带 `acl_version`、`source_acl_hash`
- [ ] 源系统角色→本系统角色映射表可编辑；映射冲突告警
- [ ] 权限变更后 5 分钟内调用 `updatedocumentpermissions` 等价 API 更新索引投影
- [ ] ACL 模拟器（§11.4.4）支持「以用户 X 预览本库检索结果」，与 Chunk ACL 校验一致
- [ ] 处理日志（§10.10.2）记录：`source_record_id`、`chunk_ids`、`acl_version`、`policy_version`、`sync_timestamp`
- [ ] 合规导出（US-1.7 扩展）可含权限矩阵与 ACL 变更记录（近 30 天）
...
| US-1.13 新鲜度治理 | — | 无 | ① `freshness_tier` + 陈旧检测任务；② 认证工作流；③ stale_retrieval 指标 |
| US-1.14 分块质量运营 | `api/apps/chunk_app.py` — Chunk CRUD | Chunk 列表/编辑 API | ① 合并拆分排除；② overlap 元数据；③ 增量重嵌；④ 检索测试标注回流 |
| US-1.15 健康度归因 | `rag/flow/indexing/` — 索引状态 | 单向量+全文状态 | ① 五维健康分聚合；② 失败阶段归因；③ 与 Hub/监控同源 |
| US-1.16 ACL 保真 | `common/connector/` | 连接器框架 | ① sync_mode batch/live；② acl_version 投影；③ 角色映射与模拟器 |
...
| Epic / 能力域 | 已落地（🟢） | 部分（🟡） | 未开始（⚪） |
|--------------|-------------|-----------|------------|
| 知识库管理 | 列表、创建向导、回收站、配置五 Tab、子导航、**权限/数据源/导出**、**五通道检索测试** | 文档/分块/索引详情页 | 文档版本 §3.9；**新鲜度/分块质量/健康度/ACL 保真**（US-1.13–1.16 设计已入 PRD，待原型） |
| RAG3 增强索引 | **Wiki / PageIndex / GraphRAG Hub** 五/六 Tab、文档→详情下钻 | 真实 API、G6 图谱 | — |
| 智能对话 | 三栏、设置、Trace、**Trace 深链链路追踪**、答案对比、纠错 | 查询增强面板 | 高级语法 §4.6 |
| 搜索 / Agent | 搜索四 Tab+设置；Agent 画布+Dataflow | — | React Flow 真实编辑 |
| 评测运营 | 仪表盘、任务、A/B、数据集、满意度、**成本、回放、路由学习** | — | — |
| 工作台 | 四分段 Tab、双行统计、RAG3 Widget、Wiki/PI 首屏、排行榜 | — | — |
| 系统管理 | **流水线配置、查询路由、系统监控、融合/检索/生成策略、链路追踪** | 用户/角色/审计/安全/模型 | Prompt/灰度/备份 §6.5–6.8 |
