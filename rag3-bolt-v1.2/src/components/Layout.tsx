...
import {
  ...
  ChevronRight, LogOut, Sun, Moon, Search, Zap, Home,
  Bot, DollarSign, Lock, Cpu, Activity, BookOpen, Layers
} from 'lucide-react';
...
const navItems: NavItem[] = [
  ...
  { key: 'wiki-manage', label: 'Wiki 管理', icon: <BookOpen size={18} />, page: 'kb-wiki-manage' },
  { key: 'pageindex-manage', label: 'PageIndex 管理', icon: <Layers size={18} />, page: 'kb-pageindex-manage' },
...
const KB_PAGES = ['kb-list', 'kb-detail', 'kb-documents', 'kb-chunks', 'kb-index-status', 'kb-settings', 'kb-retrieval-test', 'kb-wiki', 'kb-pageindex-tree'];
const EVAL_PAGES = ['eval-dashboard', 'eval-tasks', 'eval-ab-test', 'eval-cost', 'eval-replay'];
...
export function Sidebar({ collapsed, currentPage, onNavigate }: SidebarProps) {
  ...
  return (
    <aside
      className={`flex flex-col bg-white border-r border-gray-200 transition-all duration-300 flex-shrink-0 ${collapsed ? 'w-16' : 'w-56'}`}
    >
      {/* Logo */}
      <div className={`flex items-center gap-2.5 h-12 border-b border-gray-100 flex-shrink-0 ${collapsed ? 'justify-center px-2' : 'px-4'}`}>
        <div className="w-7 h-7 bg-blue-600 rounded-lg flex items-center justify-center flex-shrink-0">
          <Zap size={14} className="text-white" />
        </div>
        {!collapsed && (
          <div className="leading-tight">
            <div className="text-sm font-bold text-gray-900">RAG 3.0</div>
            <div className="text-[10px] text-gray-400">企业知识库系统</div>
          </div>
        )}
      </div>
        ...
        {navItems.map(item => {
            ...
            return (
              <div key={item.key}>
                <button
                  ...
                  className={`w-full flex items-center gap-3 py-2.5 text-sm transition-colors ${collapsed ? 'justify-center px-0' : 'px-4'} ${childActive ? 'text-blue-700' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900'}`}
                >
                  ...
                  <div className="bg-gray-50/70">
                    {item.children.map(child => (
                      <button
                        ...
                        className={`w-full flex items-center gap-2.5 pl-9 pr-4 py-2 text-sm transition-colors ${child.page === currentPage ? 'bg-blue-50 text-blue-700 border-r-2 border-blue-600 font-medium' : 'text-gray-600 hover:bg-gray-100 hover:text-gray-900'}`}
                      >
                        {child.icon}
          ...
          return (
            <button
              ...
              title={collapsed ? item.label : undefined}
              className={`w-full flex items-center gap-3 py-2.5 text-sm transition-colors ${collapsed ? 'justify-center px-0' : 'px-4'} ${isActive ? 'bg-blue-50 text-blue-700 border-r-2 border-blue-600 font-medium' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900'}`}
            >
              <span className="flex-shrink-0">{item.icon}</span>
...
export function TopBar({ collapsed, onToggle, onThemeToggle, theme, currentUser, onLogout, currentPage }: TopBarProps) {
  ...
  return (
    <header className="h-12 bg-white border-b border-gray-200 flex items-center px-4 gap-3 flex-shrink-0 relative z-30">
      <button onClick={onToggle} className="p-1.5 rounded hover:bg-gray-100 text-gray-500 flex-shrink-0">
      ...
      <div className="flex items-center gap-1.5 text-sm flex-1 min-w-0">
        {crumbs.map((c, i) => (
          <span key={i} className="flex items-center gap-1.5 min-w-0">
            ...
            <span className={`truncate ${i === crumbs.length - 1 ? 'text-gray-800 font-medium' : 'text-gray-500'}`}>{c}</span>
          </span>
        ))}
      </div>
      ...
      <div className="relative hidden md:flex items-center flex-shrink-0">
        ...
        <input
          ...
          className="pl-7 pr-3 py-1.5 text-xs bg-gray-50 border border-gray-200 rounded-lg w-52 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
        />
      </div>

      <div className="relative flex-shrink-0">
        <button onClick={() => { setShowNotif(p => !p); setShowUserMenu(false); }} className="relative p-1.5 rounded hover:bg-gray-100 text-gray-500">
          <Bell size={16} />
          <span className="absolute top-1 right-1 w-1.5 h-1.5 bg-red-500 rounded-full"></span>
        </button>
        {showNotif && (
          <div className="absolute right-0 top-10 w-72 bg-white border border-gray-200 rounded-xl shadow-xl z-50">
            <div className="px-4 py-2.5 border-b border-gray-100 font-semibold text-sm text-gray-800">通知中心</div>
            {[
      ...
      <button className="p-1.5 rounded hover:bg-gray-100 text-gray-500 flex-shrink-0 hidden sm:block">
        <HelpCircle size={16} />
      </button>

      <button onClick={onThemeToggle} className="p-1.5 rounded hover:bg-gray-100 text-gray-500 flex-shrink-0">
      ...
      <div className="relative flex-shrink-0">
        ...
        {showUserMenu && (
          <div className="absolute right-0 top-10 w-48 bg-white border border-gray-200 rounded-xl shadow-xl z-50 overflow-hidden">
...
export function StatusBar() {
  return (
    <div className="h-6 bg-gray-50 border-t border-gray-200 flex items-center px-4 gap-4 text-[10px] text-gray-500 flex-shrink-0">
